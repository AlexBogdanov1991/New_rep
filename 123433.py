...
def set_quality(message):
    ...
    try:
        ...
        if 1 <= quality <= 10:
            ...
            bot.reply_to(message, 'Пожалуйста, введи��е качество сна от 1 до 10.')
...
def status(message):
    ...
    if user_id in user_data and user_data[user_id] and user_data[user_id][-1]['end_time'] is None:
        bot.reply_to(message, 'Вы спите.')
    else:
        bot.reply_to(message, 'Вы не спите.')
...
def history(message):
    ...
    if user_id in user_data and user_data[user_id]:
        history_message = 'История сна:\n'
        for record in user_data[user_id]:
            if record['end_time']:
                history_message += (f"Начало: {record['start_time']}, Конец: {record['end_time']}, "
                                    f"Продолжительность: {record['duration']:.2f} часов, "
                                    f"Качество: {record['quality']}, Заметки: {record['notes']}\n")
            else:
                history_message += f"Начало: {record['start_time']} (незавершенный)\n"
        bot.reply_to(message, history_message)
    else:
        bot.reply_to(message, 'У вас нет записей о сне.')
