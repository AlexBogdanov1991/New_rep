...
def set_quality(message):
    ...
    try:
        ...
        bot.reply_to(message, 'Пожалуйста, введите качество сна о�� 1 до 10 в формате: /set_quality <1-10>.')
...
def set_notes(message):
    ...
    cursor.execute('UPDATE sleeping_records SET notes = ? WHERE id = ?', (notes, last_sleep_record[0]))
    conn.commit()
    conn.close()
...
def status(message):
    ...
    duration = last_sleep_record[4]
    quality = last_sleep_record[5]
    notes = last_sleep_record[6]
